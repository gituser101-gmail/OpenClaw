...
- Before publishing a local branch, run `pnpm git:push-preflight`. The preflight blocks protected branches, direct upstream pushes, unsafe `push.default` settings, and known local-output or credential paths before network push. Configure fork publishing with `git config remote.pushDefault <fork-remote>` and `git config push.default current`. For local-only sensitive paths that must never be published from your checkout, add repeatable entries with `git config --add openclaw.pushPreflight.forbiddenPath <path>`.
