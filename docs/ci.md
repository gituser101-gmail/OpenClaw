...
| `localization-catalogs`            | Block drift on ready same-repository PRs targeting the default branch; skip drafts, keep ready forks/other bases advisory, and enforce strict parity after refresh and for manual/release runs                        | Node-relevant changes                          |
...
3. `build-artifacts` and the locale checks overlap with the fast Linux lanes. Drafts do not run `localization-catalogs`. Once ready, the job fails a same-repository PR targeting the default branch when an adopted English source makes targets stale; forks and other bases remain advisory. A maintainer dispatch runs protected-`main` tooling against the exact PR head and commits one validated translation batch back under an exact-head lease. Fork/cross-repository merges and recovery runs use one generated follow-up PR with auto-merge disabled. Control UI and native app source PRs retain their conforming owner workflows. Generated changes, manual CI, and release prep enforce full translated/platform-generated parity.
