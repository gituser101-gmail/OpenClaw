...
- When the CLI times out waiting for a Gateway-backed agent reply, the CLI returns `status: "accepted_timeout"` with the Gateway-assigned `runId` only if the Gateway already accepted the turn. No embedded fallback is launched for an accepted run — the Gateway-owned turn continues executing under the default 48-hour deadline — which prevents duplicate execution and preserves the configured provider/model routing.
