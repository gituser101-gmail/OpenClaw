...
- `--gateway-auth password --gateway-password <password>` stores a plaintext password for password auth mode. Password auth has no onboarding ref-env variant; to keep the secret out of your shell history and process list, prefer token auth with `--gateway-token-ref-env`, or set `gateway.auth.password` as a SecretRef directly in config (see [Configuration reference](/gateway/configuration-reference)).
