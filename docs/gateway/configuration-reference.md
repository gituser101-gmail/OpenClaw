...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, OpenClaw
  ...
  Default `false`.
- `controlUi.allowedOrigins`: explicit browser-origin allowlist for Gateway WebSocket connects. Required for non-loopback browser origins, including remote or reverse-proxy clients. Same-origin requests from local clients (loopback socket, or a trusted-proxy-derived local origin) to loopback, RFC1918/link-local, `.local`, `.ts.net`, or Tailscale CGNAT hosts are accepted without enabling Host-header fallback. Non-local clients whose connection peer is on a private network (LAN/Tailnet/mDNS) are also accepted for same-origin private hosts. A non-local client on a public IP whose `Host` and `Origin` both name a private, LAN, Tailnet, or `.local` host is rejected as a Host-header spoof and must be added to `allowedOrigins` (or opt in via `dangerouslyAllowHostHeaderOriginFallback`).
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: dangerous mode that enables Host-header origin fallback for deployments that intentionally rely on Host-header origin policy. Non-local clients on a public IP that are rejected by the private-same-origin check can opt back in here, but prefer an explicit `allowedOrigins` entry; the fallback trusts the attacker-controlled `Host` header.
