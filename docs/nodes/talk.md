...
`realtime.providers.openai.apiKey` must be a direct OpenAI Platform key for OpenAI Realtime against `api.openai.com`, not an Azure AI Foundry / Azure OpenAI model key or OpenAI-compatible proxy key. Use `gateway-relay` with `azureEndpoint` + `azureDeployment` for Azure Realtime, or an endpoint-only `azureEndpoint` for a custom OpenAI-compatible Realtime proxy. Only positively identified Azure model keys and OpenAI-compatible proxy key formats are rejected locally; unknown direct-key formats pass through to OpenAI. `OPENCLAW_OPENAI_REALTIME_ALLOW_UNVALIDATED_KEY` is an emergency override if a direct OpenAI Platform key is still matched by those patterns; prefer fixing the configured auth path instead of leaving this enabled.
