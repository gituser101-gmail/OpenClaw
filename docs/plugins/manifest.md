...
| `talkActivityObservation`        | `string[]` | Declares a request for anonymous process-wide Talk lifecycle and speaking-timing observation.                                        |
...
`contracts.talkActivityObservation` currently accepts `"process-wide"`. Declare it only for ambient UI that intentionally observes anonymous Talk lifecycle, state, and speaking timing across sessions. The declaration does not authorize access by itself: installed plugins must also be explicitly enabled by the operator through `plugins.entries.<id>.enabled: true` (or the plugin allowlist), while bundled plugins rely on their reviewed bundled origin. The feed contains no audio, transcripts, session keys, or provider details, but timing metadata is still sensitive.
