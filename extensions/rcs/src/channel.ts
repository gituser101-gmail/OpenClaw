...
const rcsSetupContract = defineChannelSetupContract({
  fields: {
    ...
    senderId: {
      kind: "string",
      cli: { flags: "--sender-id <id>", description: "Twilio RCS sender ID" },
    },
    transport: {
      kind: "choice",
      choices: ["rcs-only", "rcs-preferred"],
      cli: { flags: "--transport <mode>", description: "RCS transport mode" },
    },
    defaultTo: {
      kind: "string",
      cli: { flags: "--default-to <e164>", description: "Default RCS recipient" },
    },
    webhookPath: {
      kind: "string",
    ...
    statusCallbacks: {
      kind: "boolean",
      cli: { flags: "--status-callbacks", description: "Enable RCS status callbacks" },
    },
    dmPolicy: {
      kind: "choice",
