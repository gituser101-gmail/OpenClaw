  ...
  return {
    ...
    rewriteTranscriptEntries: async (request) => {
      ...
      const rewriteSessionManagerEntries = () => {
        ...
        // to acquire it, so a late run does not persist over the transcript the
        // resumed foreground turn now owns.
        if (params.maintenanceFence?.tripped) {
          return fencedTranscriptRewriteResult();
        }
      ...
      // read. Tracking the whole call — the lock-wrapped rewrite plus the
      // transcript publish — keeps it bounded: a fenced no-op resolves
