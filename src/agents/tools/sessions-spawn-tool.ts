    ...
    tools: Type.Optional(
      Type.Union(
        [
          Type.Literal("none", {
            description:
              'Run the child with zero callable model tools. For embedded runtime="subagent" only.',
          }),
          Type.Object(
            {
              allow: Type.Optional(
                Type.Array(Type.String(), {
                  description: "Allowed tool names (deny wins; tool groups/globs supported).",
                }),
              ),
              deny: Type.Optional(
                Type.Array(Type.String(), {
                  description: "Denied tool names (deny wins over allow).",
                }),
              ),
            },
        ...
        {
          description:
            'Per-spawn tool policy for embedded runtime="subagent" runs. Omit for the default tool set; "none" disables all model tools; { allow, deny } narrows the child set (deny wins). Persistent spawns store the policy on the child session and reuse it on follow-up turns. The policy is immutable once set. ACP and unsupported CLI-backed runs reject this field.',
        },
      ),
    ),
