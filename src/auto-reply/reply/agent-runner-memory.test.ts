...
describe("runMemoryFlushIfNeeded", () => {
  ...
  it.each([
    ...
    "continues the reply when required preflight compaction fails with a transient %s reason",
    async (reason) => {
      const sessionFile = path.join(rootDir, "session.jsonl");
      await fs.writeFile(
        sessionFile,
  ...
  it.each(["400 bad request", "401 unauthorized", "403 forbidden"])(
    "fails the reply when required preflight compaction fails with a non-retryable %s reason",
    async (reason) => {
      const sessionFile = path.join(rootDir, "session.jsonl");
      await fs.writeFile(
        sessionFile,
      ...
      await expect(
        runPreflightCompactionIfNeeded({
          ...
          isHeartbeat: false,
          replyOperation: createReplyOperation(),
          onCompactionNotice: vi.fn(),
        }),
      ).rejects.toThrow();
    },
  );

  it("continues the reply when required preflight compaction cannot compact due to a transient provider error", async () => {
    const sessionFile = path.join(rootDir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      `${JSON.stringify({ message: { role: "user", content: "x".repeat(5_000) } })}\n`,
    ...
    compactEmbeddedAgentSessionMock.mockResolvedValueOnce({
      ok: true,
      compacted: false,
      reason: "provider 500 internal server error",
    });
    const sessionEntry: SessionEntry = {
      sessionId: "session",
      sessionFile,
