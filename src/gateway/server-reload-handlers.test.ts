...
describe("gateway hot reload model state", () => {
  it("keeps a real supervised on-exit child alive exactly once across gateway hot reload", async () => {
    ...
    await writeFile(childScriptPath, childScript, "utf8");
  ...
  it("settles terminal exit persistence before stopping and starting reload schedulers", async () => {
    const order: string[] = [];
    const terminalPersistence = createDeferredVoid();
    const exitWatchers = {
      reconcile: vi.fn(),
      cancel: vi.fn(),
      cancelAll: vi.fn(),
    ...
    const activateExitWatchers = vi.fn(async () => {
      order.push("wait-for-terminal-persistence");
    ...
    const reload = withGatewayRestartSignal(async () => {
      await applyHotReload(createCronRestartPlan(), nextConfig);
    });
    await vi.waitFor(() => expect(activateExitWatchers).toHaveBeenCalledOnce());
    expect(stopCronForHotReload).not.toHaveBeenCalled();
    expect(newCron.start).not.toHaveBeenCalled();
