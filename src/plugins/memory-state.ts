...
type MemoryPluginRuntimeRegistration = {
  pluginId: string;
  runtime: MemoryPluginRuntime;
};
...
function getMemoryCapabilityForPlugin(pluginId: string): MemoryPluginCapability | undefined {
  return memoryPluginState.capabilities.find((registration) => registration.pluginId === pluginId)
    ?.capability;
}
...
function buildSynchronousMemoryPromptSection(
...
export async function listActiveMemoryPublicArtifacts(params: {
  ...
  const selectedRegistration =
    resolveSelectedMemoryCapability(params) ?? memoryPluginState.capability;
  const publicArtifactRegistration =
    selectedRegistration?.capability.publicArtifacts || !memoryPluginState.capability
      ? selectedRegistration
      : memoryPluginState.capability;
  const pluginId = publicArtifactRegistration?.pluginId;
  const listed =
    (await publicArtifactRegistration?.capability.publicArtifacts?.listArtifacts(params)) ?? [];
